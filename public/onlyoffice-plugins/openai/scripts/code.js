...
(function(window, undefined){
	...
	// Load server-wide config (pre-configured API key + endpoint for all users)
	(function loadServerConfig() {
		fetch('../server-config.json')
			...
			.then(function(cfg) {
				...
				if (cfg.maxTokens) maxLen = cfg.maxTokens;
				if (cfg.apiKey) {
					ApiKey = cfg.apiKey;
					bHasKey = true;
					localStorage.setItem('OpenAIApiKey', cfg.apiKey);
				}
			})
			.catch(function() {});
	})();
	...
	function checkApiKey() {
		if (!ApiKey) ApiKey = localStorage.getItem('OpenAIApiKey') || '';
		if (!ApiKey.length) {
			bHasKey = false;
		} else {
			bHasKey = true;
	...
	function messageHandler(modal, message) {
		switch (message.type) {
			case 'onWindowReady':
				...
				modal.command('onBaseUrl', baseUrl);
				break;
